...
Function submit_scripted_db_dir ($scripted_db_directory)
{
    ...
    $changes = ( snapshot_commit -snapshot_tag:"$($scrptd.'dttm_human')" -remove_snapshot_path -clear_repository_after_commit -local_repository_path:($scrptd.'scm_db_path') -local_snapshot_path:($scrptd.'path') -snapshot_commit_message:$commit_msg )
